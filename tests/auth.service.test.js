...
describe("Auth Service - Password Reset", () => {
    beforeEach(() => {
        ...
        // Prisma transaction içindeki mock işlemleri çalıştırır
    ...
    test(
        ...
        async () => {
            ...
            expect(
                typeof createCall.data.tokenHash
            ).toBe("string");

            expect(
                createCall.data.tokenHash
            ).toHaveLength(64);

            expect(
                createCall.data.expiresAt
            ).toBeInstanceOf(Date);
            ...
            expect(typeof realToken).toBe(
                "string"
            );

            // randomBytes(32) hex formatında 64 karakter üretir
            expect(realToken).toMatch(
                /^[a-f0-9]{64}$/
            );
    ...
    test(
        ...
        async () => {
            ...
            emailService.sendPasswordResetEmail
                .mockRejectedValue(
                    new Error("SMTP bağlantı hatası")
                );
    ...
    test(
        "resetPassword - geçerli token ile şifreyi güncellemeli ve reset tokenlarını silmeli",
        async () => {
            const resetToken =
                "a".repeat(64);
    ...
    test(
        "resetPassword - süresi dolmuş tokenı silmeli ve 400 hata vermeli",
        async () => {
            const resetToken =
                "b".repeat(64);
